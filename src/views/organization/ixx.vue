...
<template>
  <div class="user-page">
    ...
    >
      ...
      <el-table-column align="center" label="ixx-id" width="150">
        <template slot-scope="scope">
          <span>{{ scope.row.ixx_id}}</span>
        </template>
      </el-table-column>
...
import { getIxxList } from "@/api/organization";
...
export default {
  ...
  methods: {
    ...
    getdata(parameter) {
      ...
      parameter.sidx = "create_time";
      parameter.status = 1;
      getIxxList(parameter).then(response => {
        this.loading = false;
        
        this.total = response.data.count;
        this.ixxList = response.data.ixxList;
        if(this.total<this.listQuery.rows){
          this.hidden = true
        }
      });
    },
