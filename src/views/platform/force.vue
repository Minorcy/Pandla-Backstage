...
<template>
  <div class="user-page">
    ...
    >
      ...
      <el-table-column align="center" label="魔法原力" width="150">
        <template slot-scope="scope">
          <span>{{ scope.row.magic }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="永久原力" width="150">
        <template slot-scope="scope">
          <span>{{ scope.row.ever}}</span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getdata(parameter) {
      ...
      getForceList(parameter).then(response=>{
        console.log(response)
        this.loading = false;
        this.total = response.data.count;
        this.forceData = response.data.forceList;
        })
    },
